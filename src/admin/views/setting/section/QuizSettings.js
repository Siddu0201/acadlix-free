...
import { DeleteTemplateById, GetTemplates, UpdateTemplateById } from '@acadlix/requests/admin/AdminTemplateRequest';
...
const QuizSettings = (props) => {
  ...
  const getQuizTemplates = GetTemplates("quiz");
  ...
  useEffect(() => {
    ...
    if (getQuizTemplates?.data?.data?.templates?.length > 0) {
      methods.setValue("quiz_templates", getQuizTemplates?.data?.data?.templates);
    }
  }, [getCategories?.data?.data, getLanguages?.data?.data, getSubjects?.data?.data, getQuizTemplates?.data?.data]);

  if (getCategories?.isFetching || getLanguages?.isFetching || getSubjects?.isFetching || getQuizTemplates?.isFetching) {
    return (
      <Box>
        <CircularProgress />
      </Box>
  ...
  console.log("methods watch", methods.watch("quiz_templates"));
  return (
    <Card>
      <CardContent>
        <Box>
          <CategorySettings methods={methods} />
          <QuizTemplateSettings methods={methods} />
...
const QuizTemplateSettings = ({ methods }) => {
  const updateTemplateMutation = UpdateTemplateById(methods?.watch("quiz_template_id"));
  const deleteTemplateMutation = DeleteTemplateById(methods?.watch("quiz_template_id"));

  const handleTemplateAction = () => {
    const templateId = methods?.watch("quiz_template_id");

    if (templateId === null) {
      toast.error(__("Please select template.", "acadlix"));
      return;
    } 
  ...
  const handleUpdateTemplate = () => {
    if (methods?.watch("quiz_template_name") === "") {
      toast.error(__("Please enter template name.", "acadlix"));
      return;
    }

    if (methods?.watch("quiz_templates")?.filter(c => c?.id !== methods?.watch("quiz_template_id"))?.find((c) => c?.name?.toLowerCase() === methods?.watch("quiz_template_name")?.toLowerCase())) {
      toast.error(__("Template name is already exist.", "acadlix"));
      return;
    }

    if (methods?.watch("quiz_template_id") === null) {
      toast.error(__("Please select template.", "acadlix"));
      return;
    }

    updateTemplateMutation?.mutate({
      name: methods?.watch("quiz_template_name"),
      type: 'quiz',
    }, {
      onSuccess: (data) => {
        methods.setValue("quiz_template_name", "");
        methods.setValue("quiz_template_id", null);
        methods?.setValue("quiz_templates", data?.data?.templates);
        toast.success(__("Template updated successfully.", "acadlix"));
      },
    })
  }
  ...
  const handleDeleteTemplate = () => {
    ...
    if (confirm(__("Do you really want to delete this template?", "acadlix"))) {
      deleteTemplateMutation?.mutate({ type: 'quiz' }, {
        onSuccess: (data) => {
          methods.setValue("quiz_template_name", "");
          methods.setValue("quiz_template_id", null);
          methods?.setValue("quiz_templates", data?.data?.templates);
          toast.success(__("Template deleted successfully.", "acadlix"));
        },
      });
    }
  }
  return (
    <React.Fragment>
      ...
      >
        <Typography variant="h6">{__("Quiz Templates", "acadlix")}
        </Typography>
        <Divider />
      </Box>
      <Grid container spacing={{
          ...
          >
            ...
            <Grid size={{ xs: 12, sm: 8 }}>
              <Autocomplete
                size="small"
                value={
                  methods?.watch("quiz_template_id") !== null
                    ? methods?.watch("quiz_templates")?.find(
                      (p) =>
                        p?.id ===
                        Number(methods?.watch("quiz_template_id"))
                    )
                    : null
                }
                options={methods?.watch("quiz_templates")?.length > 0 ? methods?.watch("quiz_templates") : []}
                getOptionLabel={(option) =>
                  `${option?.name} ${option?.default ? __(" (Default)", "acadlix") : ""}` || ""
                }
                isOptionEqualToValue={(option, value) => {
                ...
                renderInput={(params) => (
                  <TextField
                    ...
                    inputProps={{
                      ...params.inputProps,
                      autoComplete: "quiz_template",
                    }}
                    label={__("Select Template", "acadlix")}
                  />
                )}
                onChange={(_, newValue) => {
                  methods?.setValue(
            ...
            </Grid>
            {
              methods?.watch("quiz_template_id") !== null && (
                <>
                  <Grid size={{ xs: 12, sm: 4 }}>
                    <CustomTypography>
                      {__("Edit/Delete Template", "acadlix")}
                    </CustomTypography>
                  </Grid>
                  <Grid size={{ xs: 12, sm: 8 }}>
                    <Box sx={{
                    ...
                    >
                      <CustomTextField
                        fullWidth
                        name="quiz_template_name"
                        size="small"
                        label={__("Enter template name", "acadlix") + " *"}
                        value={methods?.watch("quiz_template_name") ?? ""}
                        onChange={(e) => {
                          methods?.setValue("quiz_template_name", e?.target?.value, {
                            shouldDirty: true,
                          });
                        }}
                        onKeyDown={(e) => {
                      ...
                      <>
                        {
                          hasCapability("acadlix_edit_template") &&
                          <Button
                        ...
                        {
                          hasCapability("acadlix_delete_template") &&
                          <Button
