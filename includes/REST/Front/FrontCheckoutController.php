...
class FrontCheckoutController
{
    ...
    public function post_checkout_razorpay($request)
    {
        ...
        if ($request->get_param("payment_method") != "razorpay") {
            return new WP_Error(__('Unacceptable payment gateway', ACADLIX_TEXT_DOMAIN), __('Unacceptable payment gateway', ACADLIX_TEXT_DOMAIN), array('status' => 404));
        }

        if (empty($request->get_param("order_items")) && count($request->get_param("order_items")) == 0) {
            return new WP_Error(__('No order found', ACADLIX_TEXT_DOMAIN), __('No order found', ACADLIX_TEXT_DOMAIN), array('status' => 404));
        }
