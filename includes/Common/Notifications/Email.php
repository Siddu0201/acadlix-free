...
if (!class_exists(__NAMESPACE__ . '\\Email')) {
  class Email
  {
    ...
    public function handleCourseCompletionEmail($course_id = 0, $user_id = 0)
    {
      ...
      $certificate_url = '';
      if ($certificate) {
        $certificate_url = add_query_arg('certificate_id', $certificate->reference_id, get_permalink(acadlix()->helper()->acadlix_get_option('acadlix_certificate_page_id')));
      }
      $r = [
      ...
      $enable_certificate = $course->rendered_metas['enable_certificate'] ?? false;
      if ($enable_certificate && $certificate_url) {
        $r['certificate_url'] = $certificate_url;
      }
