...
// Prevent class re-declaration when another plugin loads a class with the same FQCN.
if (!class_exists(__NAMESPACE__ . '\\Razorpay')) {
    class Razorpay implements PaymentGatewayInterface
    {
        ...
        public function verifyWebhook(array $data): WP_REST_Response|WP_Error
        {
            try {
                ...
                $payload = $data['stream'] ?? '';
                $signature = $data['headers']['razorpay_signature'] ?? '';
